...
- **Bicubic resize.** PyTorch `F.interpolate(mode='bicubic', align_corners=False)` uses cubic convolution with a=-0.5. `vImageScale_ARGB8888` + `kvImageHighQualityResampling` approximates Lanczos, not cubic. Option 1: use CoreImage `CIFilter.bicubicScaleTransform` (filters with CoreImage's bicubic kernel; verify match). Option 2: ship a custom Swift bicubic kernel (Mitchell-Netravali with B=0, C=0.5 = cubic with a=-0.5). Fallback is option 2 if option 1 diverges > 0.01/pixel on a 5-image test.
...
| `MUSIQPreprocessor.padToMaxSeqLen` | 200×200 image, scale=224 → 49 active patches, 0 pads. 100×100 image, scale=224 → 16 active, 33 zero pads (pad rows sum to 0). |
...
| Test | Assertion |
|---|---|
| `test_convert_musiq_deterministic` | Running `convert_musiq.py` twice produces byte-identical `.mlpackage` SHA-256. Catches dropout-at-trace bugs. |
